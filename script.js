document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchHistory() {
        try {
            ...
            if (res.ok) {
                ...
                // Reverse because we prepend to list, but API orders by newest first
    ...
    async function handleAction() {
        if (gameState === 'idle') {
            if (betPlacedForNextRound) {
                // Cannot cancel bet easily since it's already recorded in DB,
                // but for simplicity we could allow it if we implemented an endpoint.
                // For now, let's say bets cannot be cancelled once placed.
                alert('Bets cannot be cancelled once placed.');
            } else {
                // Place bet
                ...
                actionBtn.textContent = 'PLACING BET...';
    ...
    function startIdlePhase() {
        ...
        statusMessageEl.textContent = 'Waiting for next round...';
        ...
        const countdownInterval = setInterval(() => {
            ...
            if (timeLeft <= 0) {
                ...
                statusMessageEl.textContent = `Starting in ${timeLeft}s...`;
            }
        }, 1000);
        statusMessageEl.textContent = `Starting in ${timeLeft}s...`;
    }
